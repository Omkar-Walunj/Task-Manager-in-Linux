...
f_cpu_util = Figure(figsize=(5.7,2))
add_cpu_util = f_cpu_util.add_subplot(111)
cpu_utilization_per = 0

def animate_cpu(i):

    xv_cpu_utilization.append(next(index_cpu_util))
    yv_cpu_utilization.append(cpu_utilization_per)

    add_cpu_util.cla()
    add_cpu_util.set_title("CPU")
    add_cpu_util.set_ylim(0, 100)
    add_cpu_util.set_xlim(left=i-50, right=i+50)
    # add_cpu_util.set_ylabel("Percentage")
    # add_cpu_util.set_xlabel("Time")
    add_cpu_util.set_xticklabels([])
    add_cpu_util.plot(xv_cpu_utilization, yv_cpu_utilization, color='blue')
...
f_mem = Figure(figsize=(5.7, 2))
add_mem = f_mem.add_subplot(111)
mem_utilization_per = 0

def animate_mem(i):
    xv_mem.append(next(index_mem))
    yv_mem.append(mem_utilization_per)

    add_mem.cla()
    add_mem.set_title("Memory")
    add_mem.set_ylim(0, 100)
    add_mem.set_xlim(left=i-50, right=i+50)
    # add_mem.set_ylabel("Percentage")
    # add_mem.set_xlabel("Time")
    add_mem.set_xticklabels([])
    add_mem.plot(xv_mem, yv_mem, color='green')
...
f_mem_swap = Figure(figsize=(5.7, 2))
add_mem_swap = f_mem_swap.add_subplot(111)
swap_mem_utilization_per = 0

def animate_mem_swap(i):
    xv_mem_swap.append(next(index_mem_swap))
    yv_mem_swap.append(swap_mem_utilization_per)

    add_mem_swap.cla()
    add_mem_swap.set_title("Swap Memory")
    add_mem_swap.set_ylim(0, 100)
    add_mem_swap.set_xlim(left=i-50, right=i+50)
    # add_mem_swap.set_ylabel("Percentage")
    # add_mem_swap.set_xlabel("Time")
    add_mem_swap.set_xticklabels([])
    add_mem_swap.plot(xv_mem_swap, yv_mem_swap, color='purple')
...
class GUI(Tk):
    ...
    def __init__(self):
        ...
        self.cpu = Frame(notebook, bg='darkgrey')
        ...
        self.memory = Frame(notebook, bg='darkgrey')
        self.memory.grid(row=0, column=0)

        self.io = Frame(notebook, bg='darkgrey')
        ...
        self.net = Frame(notebook, bg='darkgrey')
        ...
        notebook.add(self.memory, text="Graphs")
        notebook.add(self.io, text="I/Os")
        ...
        # ----- CPU layout-------
        c1= Label(self.cpu_tab, text='CPU', font=font)
        c1.grid(row=0, column=0, padx=20, pady=5)
        c2= Label(self.cpu_tab, text='User_time', font=font)
        c2.grid(row=0, column=1, padx=20, pady=5)
        c3= Label(self.cpu_tab, text='System_time', font=font)
        c3.grid(row=0, column=2, padx=20, pady=5)
        c4= Label(self.cpu_tab, text='Idle_time', font=font)
        c4.grid(row=0, column=3, padx=20, pady=5)
        c5= Label(self.cpu_tab, text='Total', font=font)
        ...
        utilization_label = Label(self.cpu_util_frame_1, text= "CPU Utilization", font=font)
        ...
        context_sw = Label(cpu_tab_2, text="Context Switches", font=font)
        ...
        interrupt_sw = Label(cpu_tab_2, text="Interrupts", font=font)
        ...
        canvas_mem_util = FigureCanvasTkAgg(f_mem,  self.mem_frame_2)
        canvas_mem_util.draw()
        canvas_mem_util.get_tk_widget().grid(row=1, column=0, padx=5, pady=5, sticky='nsew')
        # add figure canvas here 
        canvas_mem_swap_util = FigureCanvasTkAgg(f_mem_swap,  self.mem_frame_2)
        canvas_mem_swap_util.draw()
        canvas_mem_swap_util.get_tk_widget().grid(row=2, column=0, padx=5, pady=10, sticky='nsew')
        # # add figure canvas here
        ...
        #  --------- Network layout ---------------
        # received
        interface_net = Label(self.net_tab, text="Interfaces", font=font)
        interface_net.grid(row=0, column=0)

        r_label = Label(self.net_tab, text="Received", font=font)
        ...
        interface_net = Label(self.net_tab_2, text="Interfaces", font=font)
        interface_net.grid(row=0, column=0)

        t_label = Label(self.net_tab_2, text="Transmit", font=font)
        ...
        for counter,i in enumerate(io_labels):
            io_lab = Label(self.io_tab, text=i , font=font)
    ...
    def update_cpu(self):
        ...
        for counter, i in enumerate(cpu_list):
            if counter == (len(cpu_list) - 1):
                ...
                cpu_utilization_per = utilisation
    ...
    def update_mem(self):
        global mem_utilization_per, swap_mem_utilization_per
        ...
        mem_utilization_per = percent_mem_util
        ...
        swap_mem_utilization_per = percent_swap_mem_util
...
if __name__ == "__main__":
    task_manager = GUI()
    animate_cpu_graph = animation.FuncAnimation(f_cpu_util, animate_cpu, 1000)
    animate_mem_graph = animation.FuncAnimation(f_mem, animate_mem, 1000)
    animate_mem_swap_graph = animation.FuncAnimation(f_mem_swap, animate_mem_swap, 1000)
